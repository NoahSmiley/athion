...
function HeroFluxTile() {
  ...
  return (
    <div className="w-full h-full bg-[#050507] rounded-xl overflow-hidden border border-white/[0.06] text-xs select-none flex">
      <div className="w-[72px] shrink-0 bg-[#0a0a0d] border-r border-[#1a1a1f] pt-3 px-2">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="h-6 border-b border-[#1a1a1f] flex items-center px-2 text-[9px] text-[#71717a]"># general</div>
          ...
          {messages.map((msg, i) => (
            <div key={i} className="flex items-start gap-1.5">
              ...
              <div>
                ...
                <p className="text-[9px] text-[#ccc] leading-relaxed">{msg.text}</p>
              </div>
            </div>
          ))}
        </div>
...
function HeroIDETile() {
  return (
    <div className="w-full h-full bg-[#050507] rounded-xl overflow-hidden border border-white/[0.06] text-xs select-none flex">
      <div className="w-[28px] shrink-0 bg-[#050507] border-r border-[rgba(26,26,31,0.19)] flex flex-col items-center gap-2 pt-2">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="h-5 border-b border-[#1a1a1f] flex items-center px-2">
          <span className="text-[8px] text-[#71717a] bg-[#0e0e12] px-1.5 py-0.5 rounded-sm">main.rs</span>
        </div>
      ...
      <div className="flex-1 p-2 space-y-1.5 overflow-hidden">
        {[{ name: "minecraft-smp", metric: "8/20" }, { name: "api-prod", metric: "142 req/s" }].map((s) => (
          <div key={s.name} className="border border-[#1a1a1f] rounded-sm p-2 flex items-center gap-2">
            ...
            <span className="text-[8px] text-[#71717a] ml-auto shrink-0">{s.metric}</span>
          </div>
        ))}
      </div>
    </div>
